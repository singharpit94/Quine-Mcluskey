...
#include<vector>
 ...
 int main()
 {
    ...
    cin>>nmin;
 	...
 	sizex=nmin;
 	sizey=(nv+4);
 	vector<int> min(nmin);//vector to store the minterms
      ...
      if(s==1)
   {
    ...
    {   nd*=2;
    	...
    	s=compare(mat,arp[0],arp[1],nv,nd);//comparing the prime implicants
    	if(s==2)
    	break;
   ...
   lrc(pi,q+r+2,nmin+3);//if no essential prime implicants
